...
ARTIFACTORY_NAME='viacomartifactory'
...
//SonarAnalysis()
bat "\"${tool 'MSBuild'}\\msbuild.exe\" GameNagarro.sln /p:Configuration=Debug /p:Platform=\"Any CPU\" /p:VisualStudioVersion=15.0 /p:ProductVersion=1.0.0.${env.BUILD_NUMBER} /v:diag"
...
//bat "C:\Users\Administrator run npm"

stage 'nuget packaging'

bat 'C:/jenkins/nuget.exe pack GameNagarro/GameNagarro.csproj'

stage 'deployment'


powershell 'Publish-AzureWebsiteProject -Name nagarroapp -ProjectFile .\aspnet-get-started.csproj -Configuration Debug'
